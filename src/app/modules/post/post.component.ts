...
export class PostComponent {
  ...
  constructor(private postService: PostService) {
    this.entities = postService.index();
  }
  ...
  removeItem(item: Post) {
    ...
    // 在这段代码中，this.entities 是一个数组，filter 方法被调用在 this.entities 上。
    // 回调函数接收两个参数：entity 表示当前元素，index 表示当前元素的索引。（此时item为number,要删除的元素index）
